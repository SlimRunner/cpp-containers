...
public:
  MaxHeap();
  template <class It> MaxHeap(It begin, It end);
...
template <class T> inline MaxHeap<T>::MaxHeap() : mHeap{} {}

template <class T>
template <class It>
inline MaxHeap<T>::MaxHeap(It begin, It end) : MaxHeap{} {
  std::copy(begin, end, std::back_inserter(mHeap));
  balanceHeap();
}

template <class T> inline void MaxHeap<T>::insert(T value) {
  mHeap.push_back(value);
  heapifyNode(mHeap.size() - 1);
}

template <class T>
inline std::string MaxHeap<T>::toString(TreeStyle style, bool rounded) const {
  const size_t HEIGHT =
...
template <class T> inline void MaxHeap<T>::balanceHeap() {
  std::make_heap(mHeap.begin(), mHeap.end(), std::less<T>{});
}

template <class T> inline void MaxHeap<T>::heapifyNode(size_t index) {
  bool notSorted = true;
...
template <class T> inline bool MaxHeap<T>::hasLeft(size_t index) {
  return (index + 1) * 2 >= mHeap.size();
}

template <class T> inline bool MaxHeap<T>::hasRight(size_t index) {
